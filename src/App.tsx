...
import { getSiyaResponse, getSiyaAudio, resetSiyaSession } from "./services/geminiService";
...
export default function App() {
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isBrowserAction) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-z", sender: "Siya", text: responseText }]);
      
      if (!isMuted) {
        setAppState("speaking");
        const audioBase64 = await getSiyaAudio(responseText);
        if (audioBase64) {
      ...
      responseText = await getSiyaResponse(finalTranscript, messagesRef.current);
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-z", sender: "siya", text: responseText }]);
